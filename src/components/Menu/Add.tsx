...
const Add = ({show, handleClose, editing, refresh, chef_id, chef, restaurant_id}) => {
    ...
    const handleSubmit = async () => {
        try {
            const status = editing ? await client.updateMenuItem(menuItem) : await client.createMenuItem(menuItem);
            editing ? dispatch(updateMenuItem(menuItem)) : dispatch(addMenuItem(menuItem));
